...
class DrawDetectionBoxNode:
  """
  A custom node for detecting faces in images and drawing a detection bounding box
  """
  ...
  def INPUT_TYPES(cls):
    return {"required":{
      "images":("IMAGE",{}),
      "min_face_size":("INT",{"default":20,"min":1,'max':900}),
      "prob_threshold":("FLOAT",{"default":0.9,"min":0.0,"max":1.0}),
  ...
  def filter_and_draw_images(self,images:torch.Tensor,min_face_size:int,prob_threshold:float):
    #Saving Images
    ...
    try:
      gm,info=fr_instance.filter_images_without_face(output_dir=tmp_path_save,min_face_size=min_face_size,prob_threshold=prob_threshold,return_info=True)
    ...
    fr_instance.save_images_with_detection_box(info_dict_lst=info_dict_lst,output_dir=tmp_path_boxes)
